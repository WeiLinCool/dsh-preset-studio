...
- **全页模式** — 首页「预设配置」按钮（空对话时在预设选择旁边、有对话时在顶部预设标签旁）把同一套工作室打开为整页（经 `shell.overlay`），图谱、YAML 编辑器、画布添加菜单与检查器拥有设置弹窗给不出的空间。
...
- **Harness Graph 画布**（ReactFlow + dagre）：拖拽节点、点击查看详情、悬停高亮；选中节点后关联边以绿色粒子流高亮。
- **节点检查器**：row id / 模块 / 能力类别 / 启用状态（含 `!!js` 条件表达式原文）/ 配置表单 / 原始 JSON / 从草稿删除该行。
- **画布内添加关联节点菜单**：右键节点（或点击选中节点上的 ＋）即可按能力类别插入关联组件；菜单按分类分组，支持按名称 / 模块 / 描述搜索与分类芯片筛选，group 锚点可插入组内；空画布另有全局 ＋ 按钮。Row surgery：只重写被插入的行，行外注释与格式保留。
