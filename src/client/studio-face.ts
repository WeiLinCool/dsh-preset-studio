...
export function createPresetStudioFace(controller: PresetStudioController): () => PresetStudioSectionInjected {
  return () => ({
    ...
    actions: {
      ...
      addRowAfter: (nodeId: string, block: string) => { controller.addRowAfter(nodeId, block) },
      addRowToGroup: (nodeId: string, block: string) => { controller.addRowToGroup(nodeId, block) },
