...
/** A fixed-choice field, edited through one of the option strings. */
export function choiceField(field: string, options: readonly string[]): FieldSpec {
  return {
    field,
    format: value => typeof value === 'string' && options.includes(value) ? value : '',
    parse: (text) => {
      if (text === '') return { kind: 'clear' }
      return options.includes(text) ? { kind: 'set', value: text } : undefined
    },
  }
}
